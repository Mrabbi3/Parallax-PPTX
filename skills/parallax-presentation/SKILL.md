...
**3. Write `deck.json`.** See `reference/SCHEMA.md` for every slide type. Keep to
the ten-ish slide shapes provided rather than inventing layouts; they are tuned so
text never collides with the near plane.
...
- **Peek, don't crowd.** On dense slides the near plane should be 80% off-canvas —
  a single arc of petal at the bottom edge. Full-frame subject is for the title
  and the closing slide only.
